...
func (db *DB) GetCommitments(status string) ([]*Commitment, error) {
	query := `SELECT id, chat_jid, chat_name, person_name, person_jid, title, context, direction,
		source_quote, source_time, message_id, status, due_hint, created_at, resolved_at, is_group, favorited, resolved_by
		FROM commitments`
	args := []any{}
...
func scanCommitments(rows *sql.Rows) ([]*Commitment, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.ChatJID, &c.ChatName, &c.PersonName, &c.PersonJID,
			&c.Title, &c.Context, &c.Direction, &c.SourceQuote, &c.SourceTime,
			&c.MessageID, &c.Status, &c.DueHint, &createdAt, &resolvedAt, &isGroup, &favorited, &c.ResolvedBy); err != nil {
			return nil, err
		}
...
func (db *DB) GetCommitmentStats() (*Stats, error) {
	s := &Stats{}
	var err error
	if err = db.conn.QueryRow("SELECT COUNT(*) FROM commitments WHERE status = 'open'").Scan(&s.Open); err != nil {
		return nil, err
	}
	if err = db.conn.QueryRow("SELECT COUNT(*) FROM commitments WHERE status = 'open' AND direction = 'you_owe'").Scan(&s.YouOwe); err != nil {
		return nil, err
	}
	if err = db.conn.QueryRow("SELECT COUNT(*) FROM commitments WHERE status = 'open' AND direction = 'they_owe'").Scan(&s.TheyOwe); err != nil {
		return nil, err
	}
...
func (db *DB) GetFollowUps() ([]*Commitment, error) {
	...
	rows, err := db.conn.Query(`
		...
		ORDER BY created_at ASC`, oneDayAgo, fortyEightHoursAgo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	err := db.conn.QueryRow(`
		...
		WHERE status = 'open'
			...
			AND chat_jid NOT IN (
				SELECT DISTINCT chat_jid FROM messages
				WHERE is_from_me = 0 AND timestamp > commitments.created_at
			)`, oneDayAgo, fortyEightHoursAgo).Scan(&count)
	return count, err
}
